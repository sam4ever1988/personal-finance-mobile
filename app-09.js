...
function renderInvestments(){
 ...
 document.getElementById("invKpis").innerHTML=[["Portfolio Value",invMoney(t.value),"invNeutral"],["Total Cost",invMoney(t.cost),"invNeutral"],["Unrealized Gain / Loss",(t.pl>=0?"+":"-")+invMoney(Math.abs(t.pl)),t.pl>=0?"invGood":"invBad"],["Portfolio Return",(t.pct>=0?"+":"")+t.pct.toFixed(2)+"%",t.pct>=0?"invGood":"invBad"],["Holdings",String(invHoldings.filter(h=>h.qty>0).length),"invNeutral"]].map(x=>'<div class="invKpi"><span>'+x[0]+'</span><b class="'+x[2]+'">'+x[1]+'</b></div>').join("");
 ...
 body.innerHTML=rows.map(h=>{
  var f=invFactor(h),value=h.qty*h.price*f,cost=h.qty*h.avg*f,pl=value-cost,pct=cost?pl/cost*100:0,w=t.value?value/t.value*100:0,curr=invCurrency(h),day=Number(h.dayChange),dayPct=Number(h.dayChangePct),hasDay=h.dayChange!=null&&h.dayChangePct!=null&&Number.isFinite(day)&&Number.isFinite(dayPct),positionDay=day*h.qty*f;
  return '<tr><td><div class="invTicker">'+invEsc(h.ticker)+'</div><div class="meta">'+invEsc(h.company)+'</div></td><td><span class="invMarket '+h.market.toLowerCase()+'">'+(h.market==="SA"?"Saudi":"U.S.")+'</span></td><td><input class="invEdit" data-t="'+invEsc(h.ticker)+'" data-f="qty" type="number" min="0" step="any" value="'+h.qty+'"></td><td><input class="invEdit" data-t="'+invEsc(h.ticker)+'" data-f="avg" type="number" min="0" step="any" value="'+h.avg+'"><small>'+curr+'</small></td><td><b>'+Number(h.price).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})+'</b><small>'+curr+'</small></td><td class="'+(hasDay?(day>=0?'invGood':'invBad'):'')+'">'+(hasDay?'<b>'+(day>=0?'+':'-')+invNativeMoney(Math.abs(day),curr)+'</b><small>'+(dayPct>=0?'+':'')+dayPct.toFixed(2)+'% • Position '+(positionDay>=0?'+':'-')+invMoney(Math.abs(positionDay))+'</small>':'<span class="meta">Refresh prices</span>')+'</td><td><b>'+invMoney(value)+'</b></td><td class="'+(pl>=0?"invGood":"invBad")+'"><b>'+(pl>=0?"+":"-")+invMoney(Math.abs(pl))+'</b></td><td class="'+(pct>=0?"invGood":"invBad")+'"><b>'+(pct>=0?"+":"")+pct.toFixed(2)+'%</b></td><td>'+w.toFixed(1)+'%</td><td><div class="invRowActions"><button class="btn invTradeBtn" data-t="'+invEsc(h.ticker)+'" data-type="BUY">Buy</button><button class="btn invTradeBtn" data-t="'+invEsc(h.ticker)+'" data-type="SELL">Sell</button></div></td></tr>';
 }).join("");
...
function invSubmitForm(e){
 ...
 invSave(editId?'investment-trade-edit':'investment-trade-add');document.getElementById("invModal").classList.remove("show");renderInvestments();
}
...
function invEnsureSettlementModal(){
 ...
 document.getElementById('invSettlementClose').onclick=document.getElementById('invSettlementCancel').onclick=()=>d.classList.remove('show');document.getElementById('invSettlementForm').onsubmit=invSettleTrade;
}
function invOpenSettlement(id){var x=invTrades.find(t=>t.id===id);if(!x||x.type!=='SELL'||x.settlementStatus==='settled')return;invEnsureSettlementModal();var banks=(typeof accounts!=='undefined'?accounts:[]).filter(a=>a.type==='bank');if(!banks.length)return alert('Add a bank account in My Finance before transferring the sale proceeds.');var a=invTradeAmounts(x),sar=invRound(a.net*a.factor);document.getElementById('invSettlementTradeId').value=x.id;document.getElementById('invSettlementBank').innerHTML=banks.map(b=>'<option value="'+invEsc(b.id)+'">'+invEsc(accountName(b.id))+'</option>').join('');document.getElementById('invSettlementAmount').value=sar.toFixed(2);document.getElementById('invSettlementDate').value=new Date().toISOString().slice(0,10);document.getElementById('invSettlementSub').textContent=x.ticker+' net proceeds '+invNativeMoney(a.net,a.currency);document.getElementById('invSettlementPreview').innerHTML='This creates a real income transaction and increases the selected bank balance by <b>'+invMoney(sar)+'</b>.';document.getElementById('invSettlementModal').classList.add('show')}
