...
function renderExecutiveDashboard(){
 ...
 const otherAssets=Math.max(0,Number(financeSettings.otherAssetsValue||financeSettings.otherAssets||0)),totalAssets=bank+goldValue+investmentValue+otherAssets,totalLiabilities=loanLiability+cardUsed+installmentLiability,net=totalAssets-totalLiabilities;
...
'<svg viewBox="0 0 24 24"><path d="M3 20h18"/><path d="m5 16 5-5 4 3 6-8"/><path d="M15 6h5v5"/></svg>',
...
'<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 3v9h9"/></svg>'
];$('execKpis').innerHTML=ks.map((x,i)=>'<div class="execKpi"><span class="execKpiIcon">'+icons[i]+'</span><div class="execKpiText"><small>'+x[0]+'</small><b class="'+(i===7?(x[1]>=0?'execGood':'execBad'):'')+'">'+money(x[1])+'</b><em>'+x[2]+'</em></div></div>').join('');}
 ...
 $('execGold').innerHTML='<div class="execRow"><span>Total Weight</span><b>'+goldWeight.toFixed(2)+' g</b></div><div class="execRow"><span>Gold Value</span><b>'+money(goldValue)+'</b></div><div class="execRow"><span>Investments</span><b>'+money(investmentValue)+'</b></div><div class="execRow"><span>Next Zakat Due</span><b>'+(activeGold.length?activeGold.map(a=>goldNextDue(a)).filter(Boolean).sort()[0]||'—':'—')+'</b></div>';
 ...
 $('execAssets').innerHTML='<div class="execRow"><span>🟢 Bank Accounts</span><b>'+money(bank)+'</b></div><div class="execRow"><span>🟡 Gold</span><b>'+money(goldValue)+'</b></div><div class="execRow"><span>🔵 Investments</span><b>'+money(investmentValue)+'</b></div><div class="execRow"><span>⚪ Other Assets</span><b>'+money(otherAssets)+'</b></div>';
 ...
 const drawExecCharts=()=>{if(!$('executive'))return;
  const fallback=(id,label)=>{const cv=$(id);if(!cv)return;const box=cv.parentElement;if(box&&!box.querySelector('.execChartFallback')){const d=document.createElement('div');d.className='execChartFallback';d.textContent=label;box.appendChild(d);}};execDonut('execCreditChart',[cardUsed,cardAvail],['Used','Available'],util.toFixed(0)+'%',['#7657ff','#31d7a1']);execDonut('execAssetsChart',[bank,goldValue,investmentValue,otherAssets],['Bank','Gold','Investments','Other'],'100%',['#31d7a1','#ffc44d','#3d9cff','#95a7b4']);execDonut('execLiabilitiesChart',[loanLiability,cardUsed,installmentLiability],['Loans','Cards','Installments'],'100%',['#ff4d5f','#d91f32','#31d45b']);execGoldTrend('execGoldChart',goldValue);};
...
function renderFinancialPosition(){
 ...
 const installmentLiability=installments.filter(p=>p.status!=='completed').reduce((s,p)=>s+Math.max(0,Number(p.remainingPrincipal||p.remainingAmount||0)),0);
 const totalAssets=bank+goldValue+investmentValue+otherAssets,totalLiabilities=loans+cardUsed+installmentLiability,net=totalAssets-totalLiabilities;
 const metrics=[['Total Assets',totalAssets,'Bank + gold + investments + other assets'],['Bank / Cash',bank,banks.length+' accounts'],['Gold & Investments',goldValue+investmentValue,'Market-valued assets'],['Total Liabilities',totalLiabilities,'Cards + loans + installments'],['Credit Cards',cardUsed,cards.length+' cards'],['Net Position',net,'Assets less liabilities']];
 ...
 $('fpAssets').innerHTML=`<div class="fpRow"><span>Bank Accounts</span><b>${money(bank)}</b></div><div class="fpRow"><span>Gold Assets</span><b>${money(goldValue)}</b></div><div class="fpRow"><span>Investment Portfolio</span><b>${money(investmentValue)}</b></div><div class="fpRow"><span>Other Assets</span><b>${money(otherAssets)}</b></div><div class="fpRow"><span>Total Assets</span><b class="green">${money(totalAssets)}</b></div>`;
