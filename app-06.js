...
function mergeOneRecordIntoSection(row){
 ...
 if(idx>=0){
  ...
  try{
   if(JSON.stringify(arr[idx])===JSON.stringify(row.data))return false;
  }catch(_){}
  const copy=arr.slice();copy[idx]=row.data;setSyncArrayForSection(section,copy);return true;
 }

 setSyncArrayForSection(section,[...arr,row.data]);
 return true;
}
