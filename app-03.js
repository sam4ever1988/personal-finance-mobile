...
function syncCanonicalShell(page){
 ensureCanonicalShell();var date=document.getElementById('canonicalDate');if(date){date.innerHTML='<span>'+new Date().toLocaleDateString('en-GB',{weekday:'short',day:'2-digit',month:'short',year:'numeric'})+'</span><small class="canonicalVersion">v2.85</small>';}
...
function renderGoldAssets(){
 ...
 document.querySelectorAll('[data-edit-gold]').forEach(x=>x.onclick=()=>editGoldAsset(x.dataset.editGold));document.querySelectorAll('[data-sell-gold]').forEach(x=>x.onclick=()=>sellGoldAsset(x.dataset.sellGold));document.querySelectorAll('[data-pay-zakat]').forEach(x=>x.onclick=()=>payGoldZakat(x.dataset.payZakat));document.querySelectorAll('[data-edit-zakat]').forEach(x=>x.onclick=()=>editGoldZakatPayment(x.dataset.editZakat));document.querySelectorAll('[data-delete-zakat]').forEach(x=>x.onclick=()=>deleteGoldZakatPayment(x.dataset.deleteZakat));
}
